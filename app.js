      ...
      <div class="container">
        <div class="section-head">
          <div>
            ...
            <p class="sub">Delapan kategori lengkap, dari jajanan pasar hingga catering acara.</p>
          </div>
    ...
    <div class="container">
      ...
      <div class="cat-hero">
        <div class="cat-hero-img">
          <img src="${IMG.cats[cat.id]}" alt="${cat.name}" />
        </div>
        <div class="cat-hero-text">
          <div class="label" style="color:var(--muted);margin-bottom:12px;">✦ ${cat.name.toUpperCase()}</div>
      ...
      <div class="cat-layout">
        <aside class="sidebar">
          ...
          <div class="sidebar-list">
            <div class="sidebar-item ${catId === 'all' ? 'active' : ''}" onclick="navigate('kategori')">
              <span>Semua</span>
              <span class="c">${ALL_PRODUCTS.length}</span>
            </div>
            ${CATEGORIES.map(c => `
              <div class="sidebar-item ${c.id === catId ? 'active' : ''}" onclick="navigate('kategori','${c.id}')">
                <span>${c.name}</span>
                <span class="c">${c.products.length}</span>
              </div>
            `).join('')}
          </div>

          <h3>Harga</h3>
          <div class="sidebar-list" id="priceFilter">
            <div class="sidebar-item ${currentCatFilter==='all'?'active':''}" data-filter="all" onclick="setCatFilter('all')">
              <span>Semua harga</span>
            </div>
            <div class="sidebar-item ${currentCatFilter==='low'?'active':''}" onclick="setCatFilter('low')">
              <span>< Rp 10.000</span>
            </div>
            <div class="sidebar-item ${currentCatFilter==='mid'?'active':''}" onclick="setCatFilter('mid')">
              <span>Rp 10-30rb</span>
            </div>
            <div class="sidebar-item ${currentCatFilter==='high'?'active':''}" onclick="setCatFilter('high')">
              <span>> Rp 30.000</span>
            </div>
          </div>
          ...
          <h3>Status</h3>
          <div class="sidebar-list">
            <div class="sidebar-item"><span>Tersedia</span><span class="c">${cat.products.length}</span></div>
            <div class="sidebar-item"><span>Pre-order</span><span class="c">${cat.products.length}</span></div>
          </div>
        </aside>

        <div>
          <div class="filter-chips">
            <div class="chip ${currentSort==='default'?'active':''}" onclick="setSort('default')">Urutkan</div>
            <div class="chip ${currentSort==='price-asc'?'active':''}" onclick="setSort('price-asc')">Termurah</div>
            <div class="chip ${currentSort==='price-desc'?'active':''}" onclick="setSort('price-desc')">Termahal</div>
            <div class="chip ${currentSort==='name'?'active':''}" onclick="setSort('name')">A–Z</div>
            <div class="chip ${currentSort==='best'?'active':''}" onclick="setSort('best')">Best Seller</div>
          </div>

          <div class="toolbar">
            <div class="count" id="resultCount"></div>
            <div style="display:flex;gap:8px;align-items:center;">
              <span class="muted label-sm">Tampilan:</span>
              <button class="btn btn-sm btn-secondary" id="gridBtn" onclick="toggleGridSize()">Grid</button>
            </div>
          </div>
...
function toggleGridSize() {
  currentGridSize = currentGridSize === 4 ? 3 : currentGridSize === 3 ? 2 : 4;
  const pl = document.getElementById('productList');
  if (pl) pl.style.gridTemplateColumns = `repeat(${currentGridSize}, 1fr)`;
  document.getElementById('gridBtn').textContent = `Grid ${currentGridSize}`;
}

function setCatFilter(f) { currentCatFilter = f; currentPage = 1; renderRoute(); }
function setSort(s) { currentSort = s; currentPage = 1; renderRoute(); }
...
function renderCategoryProducts(cat) {
  ...
  list.innerHTML = pageItems.length ? pageItems.map(p => renderProductCard(p)).join('') : `
    <div class="empty-state" style="grid-column:1/-1;">
      ...
      <button class="btn btn-ghost" onclick="setCatFilter('all')">Reset filter</button>
    </div>
  `;
